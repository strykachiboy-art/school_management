...
def test_create_attendance_success(json_client, admin_headers, student, term):
    payload = {
...
def test_mark_classroom_attendance_success(json_client, admin_headers, classroom, student_in_teacher_classroom, term):
    payload = {
        "term_id": term.id,
        "date": "2026-09-12",
        "attendance_data": [
            {"student_id": student_in_teacher_classroom.id, "status": AttendanceStatus.PRESENT.value}
        ]
    }
...
def test_get_attendance_by_id_success(json_client, admin_headers, attendance_record):
    response = json_client.get(f"/attendances/{attendance_record.id}", headers=admin_headers)
    assert response.status_code == 200
...
def test_get_student_attendance(json_client, admin_headers, student, attendance_record):
    response = json_client.get(f"/attendances/students/{student.id}", headers=admin_headers)
    assert response.status_code == 200
...
def test_get_classroom_attendance(json_client, admin_headers, classroom, attendance_record):
    response = json_client.get(
        f"/attendances/classrooms/{classroom.id}",
        headers=admin_headers
    )
    assert response.status_code == 200
...
def test_delete_attendance_success(json_client, admin_headers, attendance_record):
    response = json_client.delete(f"/attendances/{attendance_record.id}", headers=admin_headers)
    assert response.status_code == 200
...
def test_get_attendance_summary(json_client, admin_headers, student, attendance_record):
    response = json_client.get(
        f"/attendances/students/{student.id}/summary",
        headers=admin_headers
    )
    assert response.status_code == 200
